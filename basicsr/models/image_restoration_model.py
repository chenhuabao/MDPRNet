...
class ImageRestorationModel(BaseModel):
    ...
    def feed_data(self, data, is_val=False):
        ...
        if 'sinogram_gt' in data:
            self.sinogram_gt = data['sinogram_gt'].to(self.device)
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        # preds, stage1_img = self.net_g(self.lq)
        preds, stage1_img, stage0_img= self.net_g(self.lq)
        if not isinstance(preds, list):
            preds = [preds]
        if not isinstance(stage1_img, list):
            stage1_imgs = [stage1_img]
        if not isinstance(stage0_img, list):
            stage0_imgs = [stage0_img]
        ...
        if self.cri_pix:
            ...
            for pred in stage1_imgs:
                l_stage1 += self.cri_pix(pred, self.gt)
            for pred in stage0_imgs:
                l_stage0 += self.cri_pix(pred, self.gt)

            # print('l pix ... ', l_pix)
            l_total += l_pix + l_stage1 + l_stage0
            loss_dict['l_pix'] = l_pix
            loss_dict['l_stage1'] = l_stage1
            loss_dict['l_stage0'] = l_stage0

            # l_total += l_pix
            # loss_dict['l_pix'] = l_pix

            # l_total += 0.5*l_stage1
            # loss_dict['l_stage1'] = 0.5*l_stage1
        
        # direction loss
        # if direction_loss is not None:
        #     l_total += 100*direction_loss 
        #     loss_dict['l_direction'] = 100*direction_loss
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                # pred, stage1_img = self.net_g(self.lq[i:j])
                if isinstance(pred, list):
                    pred = pred[-1]
                # if isinstance(stage1_img, list):
                #     stage1_img = stage1_img[-1]
                # print(f'stage1_img shape: {stage1_img.shape}')
                # print(f'pred shape: {pred.shape}')
                # outs.append(stage1_img.detach().cpu())
